...
{
    char screen_text[10]; // Make sure this is large enough to hold the formatted string
    sprintf(screen_text, "%.1f° C", temp); // Convert float to string with 1 decimal places

    // Replace the decimal point with a comma
    char* comma_pos = strchr(screen_text, '.');
    if (comma_pos)
    {
    ...
    display.setFont(&Roboto_Black48pt7b);
